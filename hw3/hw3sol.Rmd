...
**Solution**: The following pipe completes the requested steps and saves the final tibble, which has admissions data, patient data, chart events, lab events, and a "death in 30 days" indicator for the first ICU stay of each patient.

```{r}
icustays_tble %>% 
  group_by(subject_id) %>%
  #filter out only first observation per patient
  filter(rank(intime) == 1) %>%
  ungroup() %>%
  #join in admissions and patients data tables
  left_join(admissions_tble, by = c("subject_id", "hadm_id")) %>%
  left_join(patients_tble, by = c("subject_id")) %>%
  #calculate age and filter out minors
  ...
  #show SQL query and collect as tibble
  show_query() %>%
  collect() %>%
  as_tibble() %>%
  #create indicator variable for death in 30 days
  mutate(time_to_death = as.duration(deathtime - intime),
         death_in_month = ifelse(is.na(time_to_death) | time_to_death > ddays(30),
                                    FALSE, TRUE)) %>%
...
**Solution**: See `app.R` in the `mimiciv_shiny` folder for my Shiny app. 
My app has 3 main tabs, each with subtabs:  

1. `Categorical Data` tab can be used for exploring demographic and admission type variables.
    + The `Plots` subtab is for creating pie charts and bar charts.  
    + The `Table` subtab is for creating frequency tables.
2. `Quantitative Data` tab can be used for exploring numeric variables, including all chart and lab events, as well as age. 
    + The `Plots` subtab allows users to create histograms. There is an optional feature for changing the x-axis, as the default plots are often hard to interpret due to outliers. 
    + The `Tables` subtab allows users to see summary statistics for each variable.
3. `Bivariate Distributions` tab houses options for seeing how the variables are jointly distributed. 
    + The `Scatterplots` subtab is used for plotting two continuous variables. The x-axis and y-axis can optionally be adjusted, and users can add a color indicator for `death in month` or `gender`.
    + The `Boxplots` subtab can be used to explore the relationship between a categorical and a continuous variable. 

When creating the Shiny app, Dr. Zhou's lecture notes, the RStudio Shiny tutorials [accessed here](https://shiny.rstudio.com/tutorial/written-tutorial/lesson1/), and the Shiny [gallery](https://shiny.rstudio.com/gallery/) were an immense help for getting ideas and code structure for the app layout and features. 
