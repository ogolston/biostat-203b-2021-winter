...
## Note  
As with many of my classmates, I've had issues with the teaching server while working on this assignment. Particularly for Questions 6-8, I wasn't able to do as much testing and troubleshooting as I would have liked, due to frequent crashes and loss of environmental variables. 
...
I'm curious how the distribution of first care units differs from last care unit, so I plot the counts side by side. 
```{r}
icu_stays %>%
...
admissions %>%
  ...
  coord_flip() +
  labs(x = "Discharge Location", x = "Discharge Location")
```
    
Most patients get discharged to their home. However, there is a large number of `NA` values. I'm assuming this means the information was not known/recorded for those patients, but I'm not sure.
...
```
It's of interest which proportion of unique patients died at some point in the data. We can do this by seeing whether each patient ever has a non-NA `deathtime`. Fortunately, the vast majority of patients are not recorded as having died during any of their admissions.
...
I would have expected a 50/50 split, but there are acutally more women than men. Perhaps childbirth and pregnancy-related visits are part of the explanation.
...
There is a huge spike at 0, and then no observations until age 18. I have two theories: one is that the age of minors is not recorded, to protect privacy. Instead, they might be coded as having age "0." After age 18, the ages appear to be coded as normal. The other theory is that "0" is a placeholder value, for when age is not known/recorded. There is also another spike at the upper end, at 91 years old. Perhaps this is the maximium value that they record, so anyone aged 91+ is coded as being 91. 
...
This process could be repeated for the other requested lab items. Since the full `labevents` file could not be read in easily to do the above process, Dr. Zhou provided the most frequently used `itemid` for the measurements. These are displayed below:
```{r}
lab_ids <- labcodes %>%
...
My best guesses at the appropriate codes are as follows:

Measurement                        | `itemid`| Rationale
-----------------------------------|---------| ---------
Heart Rate                         | 220045  | Seems to be most general version
Blood Pressure Mean (invasive)     | 220052  | Only invasive BP mean
Blood Pressure Mean (noninvasive)  | 220181  | Only noninvasive BP mean
Systolic Blood Pressure            | 220179  | Seems most general
Body Temperature (F)               | 223761  | I'm most familiar with Fahrenheit
Sp02                               | 229862  | Sounds more general than "limit"
Respiratory Rate                   | 220210  | Seems like most general version
