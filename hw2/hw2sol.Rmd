...
To make the graph more interpretable, these outlying values are excluded. 
```{r}
icu_stays %>%
  ggplot(aes(x = los)) +
...
This process could be repeated for the other requested lab items. Dr. Zhou provided the `itemids` for the all measurements. Finally, the labs file can be filtered down to only these measurements of interest.
```{r eval=F}
labs <- labs %>%
...
Dr. Zhou conducted the above steps to create the filtered data file. This was read in and saved using the following code. `charttime` was saved as a date variable. 
```{r eval = F}
...
Search for key terms in the `label` column to find the lab measurement of interest. For example, to search for heart rate and get a vector of associated `itemid` values: 
```{r}
heart_rate_ids <- chart_codes %>%
  filter(str_detect(label, regex("heart rate", ignore_case = T)))  %>%
  .$itemid %>% 
  print()
```
Finally, assuming the `chartevents` file has been read into tibble `chart_events`, the most frequently occuring heart rate associated `itemid` can be found and saved.
```{r eval = F}
top_heart_rate_id <- chart_events %>%
  filter(itemid %in% heart_rate_ids) %>%
  count(itemid) %>%
  arrange(desc(n)) %>%
  slice(1) %>%
  .$itemid %>%
...
Dr. Zhou did the steps outlined above and provided the pre-filtered file `chartevents_filtered.csv.gz`. This was read in and saved. 
```{r eval=F}
