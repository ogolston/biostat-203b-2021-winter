...
full_icu_labs <- labs %>%
  ...
  ungroup() %>%
  #rename_at(vars(as.character(labcodes$itemid)), ~labcodes$label) %>%
  print()
```
...
Let join by all matching variables in the data, since they should match between the two datasets.
```{r} 
...
## TODO 
Finish up by doing the indicator variable for death
And changing the names of columns 
