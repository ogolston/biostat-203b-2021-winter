...
Much of the code in Question 6 through 8 was provided by Dr. Zhou. I adapted it slightly to fit my needs, but would like to acknowledge that his code provided a major basis for the final solution.
...
As with the `labevents` data, Dr. Zhou did the steps outlined above but used `awk` to select only the necessary columns and rows of the `chartevents` data. This pre-filtered file, called `chartevents_filtered.csv.gz`, was read in and saved using the following code (which was provided by Dr. Zhou):
```{r eval = F}
