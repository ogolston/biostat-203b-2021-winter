...
Dr. Zhou conducted the above steps to create the filtered data file. Additionally, he used `select` to keep only the needed columns. The resulting dataset was read in and saved using the following code. `charttime` was saved as a date variable, since `fread` was parsing it as a character. 
```{r eval = F}
...
Dr. Zhou did the steps outlined above and selected only the necessary columns in order to reduce the file size. This pre-filtered file, `chartevents_filtered.csv.gz`, was read in and saved. 
```{r eval=F}
