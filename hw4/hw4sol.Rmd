...
dead_test <- dead %>%
  anti_join(dead_train, by = "subject_id")

survive_train <- survived %>%
  sample_frac(.8)

survive_test <- survived %>%
  anti_join(survive_train, by = "subject_id")
...
colnames(train)
```
Some are irrelevant, such as admittime, anchor_year, etc. We are mostly interested in lab measurements and demographic factors. 

For the training steps, only the `train` dataset will be used.  
```{r}

mylogit <- glm(death_in_month ~ calcium + magnesium + wbc + resp_rate + ethnicity + 
               insurance + chloride + potassium + temp + heart_rate + 
               language + gender + age_at_adm + creatinine + sodium + 
               systolic_bp + marital_status + bicarbonate + glucose + 
               hematocrit + mean_bp, data = train, family = "binomial")
```
...
```{r}
drop <- c("subject_id", "hadm_id", "discharge_location", "intime", "los", "stay_id", 
          "admittime", "dischtime", "hospital_expire_flag", "anchor_year_group",
          "anchor_year", "anchor_age", "outtime")
...
### Solution: Neural Net
```{r}
x_train <- train %>% select(-death_in_month)
y_train <- train %>% select(death_in_month)
x_test <- test %>% select(-death_in_month)
y_test <- test %>% select(death_in_month)
```
