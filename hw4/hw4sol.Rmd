...
### Solution 
First, the data was partitioned into a training set (80% of data) and a testing set (20% of data). The partitioning was stratified based on outcome so that the probability of death in the training set would not randomly be higher or lower than the testing set, which could lead to poorer predictive accuracy.  
```{r}
...
This model is more conservative about making death predictions, but is more precise when it does so. 
...
```
The training set has twice as many deaths, proportionally, as the original dataset. 

Now, create the neural network architecture.
```{r}
library(keras)
...
It looks like recall is better for this meethod, but precision and accuracy are a bit reduced. 
