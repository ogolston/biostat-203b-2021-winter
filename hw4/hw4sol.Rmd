...
icu_cohort %>%
  ...
  print()
```
I'm skeptical about the numbers in the 200's, but I think 941 is the only unambiguous data entry error.
...
```
All of these values are unrealistically high, so should be discarded. There are also suspicious values below 30, but I'm not sure if those are truly data entry errors, or if they indicate a significant medical event such as severe blood loss, or even death.
...
icu_cohort %>%
  ...
  print()
```
Based on the Shiny app, values above 10 are unusual, but there is a marked dropoff at 20. 

**Magnesium**
```{r}
icu_cohort %>%
  filter(magnesium > 10 | magnesium < 1) %>%
  select(magnesium) %>%
  print()
```
...
icu_cohort %>%
  ...
  print()
```
Glucose has a very large range, and there are extreme high and low values. I suspect this is related to diabetic crises, so I decided not to discard any values. 
...
icu_cohort %>%
  ...
  select(heart_rate, respiratory_rate, death_in_month) %>%
  print()
```
...
icu_cohort %>%
  ...
  print()
```
For ethnicity, there are "NA" values in hiding - "Unknown" and "Unable to obtain." However, I don't think imputation would be valid for these values, since the other variables in the data would not be very predictive of race. 
...
Currently, miceRanger is not working for me. So I'll proceed with a temporary dataset, which drops all NA values. 
```{r}
final_data <- cleaned_cohort %>%
  na.omit() %>%
  filter(ethnicity != "UNKNOWN" & ethnicity != "UNABLE TO OBTAIN") %>%
  print()
...
There are 3 categorical variables that need to be converted using one-hot encoding. 
```{r}
final_data <- final_data %>%
  mutate(female = ifelse(gender == "F", 1, 0),
         male = ifelse(gender == "M", 1, 0),
         white = ifelse(ethnicity == "WHITE", 1, 0),
         amind = ifelse(ethnicity == "AMERICAN INDIAN/ALASKA NATIVE", 1, 0),
         ...
         death_in_month = ifelse(death_in_month == TRUE, 1, 0)) %>%
  select(-c("gender", "ethnicity", "marital_status"))
```
...
```{r}
dead <- final_data %>%
  filter(death_in_month == TRUE)

survived <- final_data %>%
  filter(death_in_month == FALSE)
...
```{r}
mylogit <- glm(death_in_month ~ ., data = train, family = "binomial")
```
...
```{r}
x_train <- nntrain %>% 
  select(-death_in_month) %>% 
  scale()

y_train <- nntrain$death_in_month

x_test <- test %>% 
  select(-death_in_month) %>% 
  scale()

y_test <- test$death_in_month
...
table(factor(predictions, 
      levels=min(test$death_in_month):max(test$death_in_month)),
      factor(test$death_in_month, levels=min(test$death_in_month):max(test$death_in_month)))
